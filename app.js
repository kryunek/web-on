...
function renderExpenseMonthSelect() {
  // busca select o input dentro del form expenseForm
  let sel = document.querySelector("#expenseForm select[name=month]");
  let inputMonth = document.querySelector("#expenseForm input[name=month]");
  // si no hay ni select ni input intenta buscar globalmente
  if (!sel && !inputMonth) {
    sel = document.querySelector("select[name=month]");
    inputMonth = document.querySelector("input[name=month]");
  }
  // Si sigue sin existir, no hacemos nada
  if (!sel && !inputMonth) return;

  // Obtener lista de meses (ventas + gastos)
  const mesesVentas = (typeof getMonthsWithSales === 'function') ? getMonthsWithSales() : [];
  const mesesGastos = (typeof getMonthsWithExpenses === 'function') ? getMonthsWithExpenses() : [];
  const mesesSet = new Set([...mesesVentas, ...mesesGastos]);
  ...
  if (sel) {
    ...
    if (meses.length === 0) {
      ...
      opt.value = defaultMonth; opt.textContent = `${mes}/${year}`;
      sel.appendChild(opt);
      ...
      if (!sel.value) sel.value = meses[0];
    }
  }

  if (inputMonth) {
    // si usas input type="month" ponemos el primer mes disponible o mes actual
    if (meses.length === 0) {
      const today = new Date();
      const mes = String(today.getMonth()+1).padStart(2,'0');
      const year = today.getFullYear();
      inputMonth.value = `${year}-${mes}`;
    } else {
      inputMonth.value = meses[0]; // el más reciente
    }
  }
}
...
function updateProductSelectByProvider() {
  ...
  if (list.length > 0) { unitField.value = list[0].unit || ''; priceField.value = list[0].price || ''; } 
}
...
function showTab(sectionId, tabId) {
  const sections = ["dashboard-section","sales-section","escandallo-section","staff-section","reservas-section","tasks-section","providers-section","orders-section","products-section"];
  sections.forEach(id => { const el = document.getElementById(id); if (el) el.style.display = "none"; });
  const target = document.getElementById(sectionId);
  if (target) target.style.display = "";
  const tabs = ["tab-dashboard","tab-sales","tab-escandallo","tab-staff","tab-reservas","tab-tasks","tab-providers","tab-orders","tab-products"];
  tabs.forEach(id => { const el = document.getElementById(id); if (el && el.classList) el.classList.remove("active"); });
...
document.getElementById("tab-reservas").onclick = () => showTab("reservas-section", "tab-reservas");
...
document.getElementById("login-form").onsubmit = function(e){
  ...
  if (doLogin(user, pass)) { errDiv.innerText = ""; showApp(); setTimeout(fadeOutLoader,350); setTimeout(appInit,100); } 
};
...
// --- TABS: robust showTab and sidebar hookup ---
(function(){
  // list of known section IDs in the app (adjust if you have different ids)
  const SECTION_IDS = [
    "dashboard-section","sales-section","escandallo-section",
    "staff-section","reservas-section","tasks-section","providers-section",
    "products-section","orders-section"
  ];

  // showTab function (overrides any previous version)
  window.showTab = function(sectionId, tabId) {
    try {
      // hide all sections
      ...
      SECTION_IDS.forEach(id => {
        const tab = document.getElementById('tab-' + id.replace('-section',''));
  ...
  // Attach listeners to sidebar items that start with id="tab-"
  function attachSidebarHandlers() {
    const all = Array.from(document.querySelectorAll('[id^="tab-"]'));
    if (!all.length) return;
    all.forEach(tab => {
      // compute target section id
      const tabId = tab.id;
      const sec = tabId.startsWith('tab-') ? tabId.replace('tab-','') + '-section' : null;
      tab.style.cursor = 'pointer';
      tab.addEventListener('click', function(e){
        e.preventDefault();
        if (sec) {
          window.showTab(sec, tabId);
        }
      });
    });
  }
  ...
  // expose function to rebind (debug)
  window.__bindTabs = attachSidebarHandlers;
})();


// --- Clean robust showTab implementation ---
(function(){
  const SECTIONS = [
    "dashboard-section","sales-section","escandallo-section",
    "staff-section","reservas-section","tasks-section","providers-section",
  ...
  window.showTab = function(sectionId, tabId){
    try{
      ...
      allTabs.forEach(t=>t.classList&&t.classList.remove("active"));
      const target=document.getElementById(sectionId);
      if(target) target.style.display="";
      const activeTab=tabId?document.getElementById(tabId):document.getElementById("tab-"+sectionId.replace("-section",""));
  ...
  function attachTabs(){
    ...
    tabs.forEach(tab=>{
      const sec=tab.id.replace("tab-","")+"-section";
      tab.style.cursor="pointer";
      tab.addEventListener("click",()=>window.showTab(sec,tab.id));
    });
  }
  if(document.readyState==="loading")document.addEventListener("DOMContentLoaded",attachTabs);
  else attachTabs();
})();
